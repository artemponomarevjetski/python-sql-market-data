            ...
            with open(os.path.join(working_dir, 'master_table.csv'), 'w') as result_file:
                ...
                for row in table_master1:
                    if i == 0:
                        ...
                        row[5] == row_prev[5]:
                            accumulated_tickers += row[3]+'collapse redundancy, '
                        else:
                            row4 = []
                            row4.append(row[0])
                            row4.append(row[1])
                            row4.append(row[2])
                            ticker_choices3 = accumulated_tickers[:-2].split(',')
                            print(ticker_choices3, accumulated_tickers,
                                  accumulated_tickers[:-2])
                            ticker_choices4 = []
...
# change True to False to use the list
            ...
            if top is None or top > 100000: # to get a meaningful missing output, top has to be None
