...
# other functions
#
def write_good_proxies(seccode_thread_set):
    local_db = pyodbc.connect("localhost","username","PassW","DB", port=3306 )
    cursor_ = local_db.cursor (pyodbc.cursors.DictCursor)
    query_seccodes = ''
    print('\n\nseccodeblock = ', len(seccode_thread_set), datetime.now())
    for s_c in seccode_thread_list[i]:
...
#
    ...
    with tqdm(total=nblocks, file=sys.stdout) as pbar:
     ...
     #   for seccodeblock in list(iterutils.chunked_iter(seccodes, block_size)):
        seccode_thread_list=list(iterutils.chunked_iter(seccodes, block_size))
        threads = []
        for i in range(8):
            print (i)
...
#
    ...
    PARSER.add_argument(
        ...
        '--nblocks',
        type=int,
        default=8, # TODO: automatically determine the number of available cores for the default value
