            ...
            with open(os.path.join(working_dir, 'master_table.csv'), 'w') as result_file:
                ...
                for row in table_master1:
                    ...
                    if str(row[0]) != str(row_prev[0]):
                        ...
                        ticker_choices4 = []
                        for t_3 in ticker_choices3:
                            if t_3 not in ticker_choices4:
                                ticker_choices4.append(t_3)
                        ticker_choices4 = sorted(ticker_choices4)
                        tickers2 = ''
                        for t_4 in ticker_choices4:
                            if t_4:
                                tickers2 += t_4+', '
                        row4.append(tickers2[:-2])
...
# change True to False to use the list
            ...
            for row in table_master3:
                tickers_in_secode = row[3].split(',')
                print(tickers_in_secode)
                for t_1 in tickers_in_secode:
                    print(t_1)
                    if t_1 not in tikers_art: # unique tickers
                        tikers_art.append(t_1)

            print('\n\nnumber of unique tickers in the master: ', len(tikers_art), datetime.now())

            if top is None or top > 1000000:
                                        ...
                                        if '-' in s_1:
                                            ...
                                            for s_2 in range(int(s_3[0]), int(s_3[1])+1):
                                                if s_2 not in seccodes: # unique seccodes
                                                    seccodes.append(s_2)
                                                    if max_seccode < int(s_2):
                                                        max_seccode = int(s_2)
                                        else:
                                            if s_1 not in seccodes: # unique seccodes
                                                seccodes.append(s_1)
                                                if max_seccode < int(s_1):
                                                    max_seccode = int(s_1)
                        l_3 += 1
#
